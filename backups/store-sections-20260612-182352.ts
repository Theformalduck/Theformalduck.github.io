...
// Urgency block: live countdown to a date, e.g. a sale end or product drop.
export interface CountdownSection extends Base {
  type: "countdown";
  heading: string; subtext: string;
  endsAt: string;            // ISO datetime the timer counts down to
  expiredText: string;       // shown once the timer hits zero
  ctaLabel?: string; ctaUrl?: string;
}
// Single-product spotlight: one product, large, with price + add to cart.
export interface SpotlightSection extends Base {
  type: "spotlight";
  heading: string;
  productId: string;         // which product to feature ("" = newest)
  blurb: string;             // optional supporting copy
  layout: "left" | "right";  // image side
}
...
export type StoreSection =
  ...
  | CountdownSection | SpotlightSection;
...
export function defaultSection(type: StoreSectionType): StoreSection {
  ...
  switch (type) {
    ...
    case "countdown":
      return {
        id, type, heading: "Sale ends soon", subtext: "Don't miss it, prices go back up when the timer hits zero.",
        endsAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
        expiredText: "This offer has ended.", ctaLabel: "Shop the sale", ctaUrl: "#products",
      };
    case "spotlight":
      return { id, type, heading: "Featured product", productId: "", blurb: "", layout: "left" };
    case "blocks":
...
export const SECTION_NATURAL: Record<StoreSectionType, { width: SectionWidth; bg: "default" | "surface" }> = {
  ...
  countdown:    { width: "normal", bg: "surface" },
  spotlight:    { width: "normal", bg: "default" },
};
...
// Deep-copy a section with fresh ids so a saved section can be inserted many
// times (ids must be unique within a page for drag-and-drop and React keys).
export function cloneSectionWithNewIds(s: StoreSection): StoreSection {
  const copy = JSON.parse(JSON.stringify(s)) as StoreSection;
  copy.id = sectionUid();
  if (copy.type === "blocks") {
    copy.blocks = (copy.blocks ?? []).map((b) => ({ ...b, id: blockUid() }));
  }
  return copy;
}
