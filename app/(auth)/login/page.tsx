...
import { Mail, Lock, Share2, Eye, EyeOff, ArrowLeft } from "lucide-react";
...
function LoginForm() {
  ...
  return (
    <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl shadow-sky-900/20 border border-white/60 p-8">
      ...
      <p className="text-gray-500 text-sm mb-8">Sign in to your creator account</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <Link href="/forgot-password" className="text-xs text-nexus-600 hover:text-nexus-700 transition-colors">
              Forgot password?
            </Link>
          </div>
          <div className="relative">
      ...
      <p className="text-center text-gray-500 text-sm mt-6">
        ...
        <Link href="/signup" className="text-nexus-600 hover:text-nexus-700 font-medium">Create one free</Link>
      </p>
    </div>
  );
}
...
export default function LoginPage() {
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(180deg, #0870b0 0%, #0f9fd8 18%, #1ab8ea 40%, #22c4ef 65%, #52d0f5 85%, #cef3fd 97%, #f0faff 100%)",
      }}
    >
      <div className="absolute inset-0 grid-pattern opacity-20" />
      <div className="absolute top-1/4 left-1/4 w-72 h-72 bg-white/20 rounded-full blur-[100px]" />
      <div className="absolute bottom-1/4 right-1/4 w-72 h-72 bg-white/15 rounded-full blur-[100px]" />

      <div className="relative w-full max-w-md px-4">
        <Link href="/" className="inline-flex items-center gap-2 text-white/80 hover:text-white text-sm mb-8 drop-shadow transition-colors">
          <ArrowLeft className="w-4 h-4" />
          Back to home
        </Link>
        <Suspense fallback={<div className="h-96 bg-white/90 rounded-2xl animate-pulse" />}>
          <LoginForm />
        </Suspense>
      </div>
    </div>
